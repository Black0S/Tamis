...
/// End to end over HTTP/2: ALPN on both sides, the bridge, filtering and injection.
///
/// The bug that kept these disabled was not in the bridge at all. Between the upstream
/// TLS handshake completing and the protocol handlers being installed, the upstream
/// pipeline had no reader — and over HTTP/2 the origin speaks first, so its SETTINGS
/// frame arrived in that window and was dropped. `NIOHTTP2Handler` then saw the peer
/// acknowledge settings it had never sent, called it a PROTOCOL_ERROR and sent GOAWAY;
/// the response arrived afterwards on a connection that was already finished, which is
/// what "frames never reach the stream channel" actually was.
///
/// Over HTTP/1.1 the same window exists and is harmless, because an origin says nothing
/// until it is asked. That is why only HTTP/2 broke, and why the fix belongs in
/// `HandshakeReporter` rather than anywhere near the bridge.
...
struct HTTP2Tests {
    ...
    @Test("an HTTP/2 origin is intercepted end to end")
    ...
    @Test("cosmetic injection works the same over HTTP/2")
    ...
    @Test("request blocking works the same over HTTP/2")
